...
sortingAlgos.forEach((elem) => {
    ...
    elem.nums.forEach((elem, index) => {
        // Create a container for each bar and its label
        ...
        // Create the label for the number below the bar
        const barLabel = document.createElement('span');
        barLabel.innerHTML = elem;
        barLabel.id = prefix + "label" + index;
...
function swapHeights(i, j, prefix) {
    ...
    // Swap the labels (numbers)
    const labeli = document.getElementById(prefix + "label" + i);
    const labelj = document.getElementById(prefix + "label" + j);

    const tempLabel = labeli.innerText;
    labeli.innerText = labelj.innerText;
    labelj.innerText = tempLabel;
}
